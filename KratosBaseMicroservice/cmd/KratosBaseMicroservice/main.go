...
func main() {
	...
	// Without defer, you have to remember to close a resource before every return statement. This is error-prone.
	defer c.Close() 
    // Loads configuration from files Crashes the app if config is invalid or missing
	if err := c.Load(); err != nil {
		panic(err)
	}
	// scan conf into bc (Bootstrap struct) Bootstrap is your root config struct contain Server , Data , Registry etc.
	var bc conf.Bootstrap
	// Maps config data into Go struct Similar to unmarshalling YAML → struct
	if err := c.Scan(&bc); err != nil {
		panic(err)
	}
    // wireApp is generated by Google Wire Responsibilities: 
	// Create DB connections, Create repositories, Create servicesCreate HTTP & gRPC servers, Create Kratos app
	app, cleanup, err := wireApp(bc.Server, bc.Data, logger)
	//Fails fast if dependency injection fails
	if err != nil {
		panic(err)
	}
	/*
